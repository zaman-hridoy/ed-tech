...
export function ExamFileViewer() {
  ...
  const isModalOpen = modal.isOpen && modal.type === "EXAM_FILE_VIEWER";

  const [loading, setLoading] = useState(false);

  const [previewUrl, setPreviewUrl] = useState(file?.data?.url);
  useEffect(() => {
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-3xl bg-white p-0 gap-y-0 border-[var(--brand-color)]">
        <DialogHeader className="flex flex-row items-center justify-between border-b p-2">
