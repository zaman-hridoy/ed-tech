...
const SharedWithMeTree = () => {
  const { data } = useSession();
  const session = data as SessionWithUserType;
  const folderState = useQuery({
    queryKey: ["share-with-me-parent-folders"],
    queryFn: () =>
      axios
        .post(
          "/content/getshareExamLibrary",
          {
            reciever_student_id: session.user?.userId,
          },
          {
            headers: {
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-x-2 text-sm font-semibold tracking-tight w-full text-slate-500 hover:text-[var(--brand-color)]">
        ...
        <Link href="/shared-with" className="text-inherit hover:underline">
          Shared with me
        </Link>
      </div>
...
export default SharedWithMeTree;
