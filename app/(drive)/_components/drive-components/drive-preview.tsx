...
const DrivePreview = () => {
  ...
  const getRenderer = () => {
    if (imageMimeTypes.includes(mimeType)) {
      return <DriveImageRenderer assetUrl={file?.data?.upload_path} />;
    } else if (videoMimeTypes.includes(mimeType)) {
      return <DriveVideoRenderer assetUrl={file?.data?.upload_path} />;
    ...
    if (imageMimeTypes.includes(mimeType)) {
      return (
        <Button
          ...
          size="sm"
          onClick={() => modal.onModalOpen("DRIVE_FILE_VIEWER", { ...file })}
        >
    ...
    <div className="p-4 space-y-8 select-none">
      ...
      <div className="space-y-2">
        <div className="text-xs font-semibold text-slate-500 flex flex-col">
          ...
          <span className="text-slate-800">{file?.data?.type}</span>
        </div>
