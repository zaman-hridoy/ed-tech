...
import { CalendarIcon, Edit3Icon, Loader2, Save } from "lucide-react";
...
import { NameUpdateModal } from "@/app/(info-and-course)/auth/add-info/_components/name-update-nodal";
    ...
    <div className="bg-white rounded-md p-4 md:p-6">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    onValueChange={(val) => {
                      field.onChange(val);
                      const stateArrList: SelectOptionType[] = states[+val + 1]
                        ?.split("|")
                        .map((s: string, i: number) => ({
                          id: i + 1,
                          name: s,
                    ...
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                          variant={"outline"}
                          className={cn(
                            "bg-white w-full pl-3 text-left font-normal",
      ...
      <div className="flex items-center justify-end gap-x-4">
        ...
        >
          ...
          Save changes
          {!isSubmitting && <Save className="text-inherit w-4 h-4" />}
        </Button>
      </div>
    </div>
  );
